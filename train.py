import sys
import time
import os #this is used to set up paths 
import math
import csv
import argparse

import numpy as np
import logging #this lib will be used for creating the logs
...
def main(args):
    ...
    if not os.path.exists(output_folder):
        #checking if the folder is already created or not
    ...
    # the main definition of the model.
    ...
    fake_prob = NNDiscriminator(d_fake_inputs, inputs_depth, sequence_length, reuse=True, scope="prob")



   # Geodesic loss

    sq_geodesic_dist = tf.squared_dist(real_prob, d_real)
    batch_loss = np.sum(sq_geodesic_dist).astype('float32')
    # Computation of Riemannian Gradient
    tangent_vec = self.metric.log(base_point=real_prob, point=d_real)
    grad_point = - 2. * tangent_vec
    inner_prod_mat = tf.inner_product_matrix(base_point=y_pred)
    ...
    # should come after the optimizer
    init_op = tf.global_variables_initializer()

    # Draws all z random intrinsic vector used in visualization
    z_rand_type = 'uniform'
    ...
    with tf.Session() as sess:
        ...
        while epoch < max_epochs:
            ...
            # Training
            #the main training of our model starts from here
...
#to calculate the best epoch
            if epoch > 20: # Ignore first couple of epochs as their results are prob uncertain.
                save_model_and_video = False
                prob_count = -1. # exclude the ground truth.
