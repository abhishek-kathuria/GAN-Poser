...
#first we compute the one-hot encoding 
#why needed? => since categories are large
# in this, first create the dummy variables of the category
#then look if we can fdrop any field (column)
...
# this creates input sequece
#how? => by comparing the skeleton structure size of pred of the last bone size
...
# remeber to refer to formula from the saved website
...
def attention(inputs, 
    ...
    # we find which parameters are trainable parameters
...
def create_cell(cell_type, num_neurons, use_residual=False):
    cell = None
    if cell_type == "lstm":
...
def generate_skeletons_with_prob(sess, g, d_real_prob, d_fake_prob, data_preprocessing, 
    ...
    # here it is used to generate skeletons according to the intrinsic parameter
